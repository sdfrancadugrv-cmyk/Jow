...
function ProdutoShopContent() {
  ...
  const [afiliadoWhatsapp, setAfiliadoWhatsapp] = useState<string | null>(null);
  const [mostraLinkWhatsapp, setMostraLinkWhatsapp] = useState(false);
  ...
  useEffect(() => {
    fetch(`/api/shop/produto/${slug}${ref ? `?ref=${ref}` : ""}`)
      .then(r => r.json())
      .then(d => { if (d.produto) setProduto(d.produto); if (d.afiliadoWhatsapp) setAfiliadoWhatsapp(d.afiliadoWhatsapp); })
    ...
    try {
      ...
      const res = await fetch("/api/shop/chat", {
        ...
        body: JSON.stringify({
          ...
          produtoSlug: slug,
          afiliadoWhatsapp: afiliadoWhatsapp || null,
          numMensagens: historico.filter(m => m.role === "user").length,
        }),
      });
      ...
      // Detecta indicação de especialista WhatsApp
      if (textoFinal.includes("INDICAR_WHATSAPP")) {
        setMostraLinkWhatsapp(true);
      }
      onFimAudio.current = () => {
        setTextoAtual(""); textoAtualRef.current = "";
        const final = textoFinal.replace(/PEDIDO_PRONTO:[^\s]*/g, "").replace(/INDICAR_WHATSAPP/g, "").trim();
  ...
  useEffect(() => {
    ...
    setEstado("falando");
    const intro = afiliadoWhatsapp
      ? "Oiêêê... Eu sou a vendedora Jennifer e tô aqui pra te ajudar! Eu consigo te ouvir, compreender e te explicar tudo sobre o produto por aqui. Ou se preferir, pode falar direto com o vendedor no WhatsApp. Fica à vontade! Tem alguma pergunta sobre esse produto?"
      : "Oiêêê... Eu sou a vendedora Jennifer e tô aqui pra te ajudar! Eu consigo te ouvir, compreender e te explicar tudo sobre o produto por aqui. Fica à vontade! Tem alguma pergunta sobre esse produto?";
    enfileirarTTS(intro);
    onFimAudio.current = () => {
            ...
            {afiliadoWhatsapp && (
              ...
              >
                <div style={{
                  ...
                  background: "#25D366", borderRadius: 10, padding: "14px 18px",
                  ...
                  <div>
                    <p style={{ color: "#fff", fontWeight: 700, fontSize: 14, lineHeight: 1 }}>Falar com o vendedor</p>
                    <p style={{ color: "rgba(255,255,255,0.8)", fontSize: 11, marginTop: 3 }}>Tire suas dúvidas no WhatsApp</p>
                  </div>
                </div>
              </a>
            )}
            ...
            <a href="/revendedor" style={{ textDecoration: "none" }}>
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 12, position: "relative" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <p style={{ color: "#FFE600", fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", marginBottom: 6 }}>AFILIADO — É DE GRAÇA</p>
                    <p style={{ color: "#fff", fontSize: 14, fontWeight: 700, lineHeight: 1.4, marginBottom: 10 }}>
                      Peça pra Jennifer vender pra você e ganhe comissão pelas vendas.
                    </p>
                    <ul style={{ color: "rgba(255,255,255,0.75)", fontSize: 12, lineHeight: 1.8, marginBottom: 6, paddingLeft: 0, listStyle: "none" }}>
                      <li>✦ Ela publica teu link</li>
                      <li>✦ Conversa por voz com o cliente</li>
                      <li>✦ Tira dúvidas</li>
                      <li>✦ Vende sozinha</li>
                    </ul>
              ...
              {textoAtual && (
                <div style={{ display: "flex", justifyContent: "flex-start" }}>
                  <div style={{ maxWidth: "80%", padding: "8px 12px", borderRadius: "12px 12px 12px 2px", background: "#f0f0f0", color: "#333", fontSize: 13, lineHeight: 1.5 }}>
                    {textoAtual.replace(/INDICAR_WHATSAPP/g, "").replace(/PEDIDO_PRONTO:[^\s]*/g, "").trim()}<span style={{ opacity: 0.4 }}>|</span>
                  </div>
                </div>
              )}
              ...
              {mostraLinkWhatsapp && afiliadoWhatsapp && (
                <div style={{ display: "flex", justifyContent: "flex-start" }}>
                  ...
                  >
                    ...
                    <span style={{ color: "#fff", fontWeight: 700, fontSize: 13 }}>Falar com o especialista</span>
                  </a>
                </div>
              )}
              <div ref={chatEndRef} />
